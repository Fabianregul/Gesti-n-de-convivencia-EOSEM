...
def dashboard(request):
    ...
    if query:
        estudiantes = (
            Estudiante.objects.filter(nombre__icontains=query) |
            Estudiante.objects.filter(codigo__icontains=query) |
            Estudiante.objects.filter(grado__icontains=query)
        )
    else:
...
def eliminar_estudiante(request, id):
    ...
    if request.method == 'POST':
        ...
        # validación contra contraseña fija
        if password_confirm == PASSWORD_ADMIN:
            estudiante.delete()
            messages.success(request, "Estudiante eliminado correctamente.")
        else:
            messages.error(request, "Contraseña incorrecta. No se pudo eliminar.")
...
def ver_reportes(request):
    ...
    if query:
        estudiante = (
            Estudiante.objects.filter(codigo=query).first()
            or Estudiante.objects.filter(nombre__icontains=query).first()
        )
